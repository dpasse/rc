import os
import json
from prefect import flow


from common.pbp.models import Game
from common.helpers.templates import TemplateService


PBP_DIRECTORY = '../data/mlb/pbp/2/'
TEST_DIRECTORY = './tasks/mlb/common/tests/docs/'

@flow(name='mlb-dump-entities', persist_result=False)
def run_dump() -> None:
    template_service = TemplateService()

    keys = set()
    text_instances = []
    for file in os.listdir(PBP_DIRECTORY):
        with open(os.path.join(PBP_DIRECTORY, file), 'r', encoding='UTF8') as pbp_input:
            game = Game(json.load(pbp_input))

        for period in game.periods:
            for event in period.events:
                description = event.desc
                entities = event.entities

                is_valid, key1 = template_service.validate(description[:], entities.body)
                if is_valid:
                    key2 = entities.type

                    key = f'{key1}_{key2}'
                    if not key in keys:
                        text_instances.append((description, entities.body))
                        keys.add(key)
                else:
                    print(f'{game.id} - {period.id} - {event.id} does not match a valid template.')

    with open(os.path.join(TEST_DIRECTORY, 'desc_to_entities.json'), 'w', encoding='UTF8') as test_instances:
        json.dump(text_instances, test_instances, indent=2)


if __name__ == '__main__':
    run_dump()
